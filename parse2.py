...
class Parser:
    ...
    def __init__(self, names, devices, network, monitors, scanner):
        ...
        [self.output_to_output, self.input_to_input, self.input_to_output,
         ...
         self.missing_symbol,
         self.section_order_error, self.device_as_name, self.keyword_as_name, self.dtype_as_name, self.invalid_output_identifier] = self.names.unique_error_codes(15)

        self.keyword = [self.scanner.DEVICES_ID,
                        self.scanner.CONNECT_ID,
                        self.scanner.MONITOR_ID]
        
        self.signal_types = [self.INPUT, self.OUTPUT] = range(2)
    ...
    # def is_output(self):
    #     """
    #     Returns True if symbol is an output, False if the name is that of an input
    #     """
    #     self.name()
    #     if self.symbol.type == self.scanner.DOT:
    #         self.symbol = self.scanner.get_symbol()
    #         if self.symbol.type == self.scanner.DTYPE_OP: # ".Q" or ".QBAR" is seen
    #             self.symbol = self.scanner.get_symbol()
    #             return True, 
    #         elif (self.symbol.type == self.scanner.KEYWORD and
    #                 self.symbol.id == self.scanner.I_ID): # ".I" is seen
    #             self.argument()
    #             return False
    #         elif self.symbol.type == self.scanner.DTYPE_IP: # ".CLK", ".DATA", ".SET", ".CLEAR" is seen
    #             self.symbol = self.scanner.get_symbol()
    #             return False
    #         else: 
    #             self.syntax_error()
    #     else: # no "." is seen, therefore must be an output
    #         return True
    ...
    def get_port_attrs(self):
        ...
        device_id = self.name() # will give None if it's not a valid name
        if self.symbol.type == self.scanner.DOT:
            self.symbol = self.scanner.get_symbol()
            if self.symbol.type == self.scanner.DTYPE_OP: # ".Q" or ".QBAR" is seen
                self.symbol = self.scanner.get_symbol()
                type = self.OUTPUT
                if self.symbol.id == self.scanner.Q_ID:
                    ...
                    self.symbol.id == self.scanner.I_ID): # ".I" is seen
                type = self.INPUT
                port_id = self.argument()
            elif self.symbol.type == self.scanner.DTYPE_IP: # ".CLK", ".DATA", ".SET", ".CLEAR" is seen
                self.symbol = self.scanner.get_symbol()
                type = self.INPUT
                if self.symbol.id == self.scanner.SET_ID:
                    port_id = self.devices.SET_ID
                if self.symbol.id == self.scanner.CLEAR_ID:
                    port_id = self.devices.CLEAR_ID
                if self.symbol.id == self.scanner.DATA_ID:
                    port_id = self.devices.DATA_ID
                else:
                    port_id = self.devices.CLK_ID
            else: 
                self.syntax_error(self.invalid_output_identifier)
        else: # no "." is seen, therefore must be an output
            type = self.OUTPUT
            port_id = "itself"

        return type, device_id, port_id
    ...
    def connection(self):
        ...
        { "," , ( input | dtype_ip ) }, ";" ;"""
        type, device_id, port_id = self.get_port_attrs()
        print(type, device_id, port_id)
        # x = self.is_output()
        # print(x)
